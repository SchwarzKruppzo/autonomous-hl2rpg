...
do
	...
	net.Receive("donateNotify", function()
		...
		elseif msg == 0 then
			...
			chat.AddText(clr, "Администрация активировала Вашу месячную подписку. Благодарим Вас за поддержку сервера AUTONOMOUS! Благодаря Вам мы станем еще лучше, чем вчера. Вы сохраните статус игрока, поддерживающего сервер до: "..timeLeft)
			chat.AddText(clr, "Для того, чтобы открыть персональный контейнер, используйте команду /хранилище!")
		end
	end)
end