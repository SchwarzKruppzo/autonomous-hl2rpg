ITEM.name = "Бутылка водки"
ITEM.description = "Высокая бутылка с винтовой крышкой. Вкус острый и чистый, с нотками зернового солода. Если много пить, вы постепенно чувствуете как вам хочется что-то сказать на русском"
ITEM.cost = 10
ITEM.model = "models/mark2580/gtav/barstuff/vodka_bottle.mdl"
ITEM.width = 1
ITEM.height = 2
ITEM.iconCam = {
	pos = Vector(213.14823913574, 102.97333526611, 6.2486262321472),
	ang = Angle(1.5123836994171, 205.78550720215, 0),
	fov = 2.0806002355042,
}

ITEM.stats.container = true
ITEM.stats.thirst = 7
ITEM.stats.hunger = 0
ITEM.stats.uses = 10

ITEM.rarity = 1
ITEM.junk = "empty_glass_bottle"

function ITEM:CustomEffect(client, uses)
	local health = client:GetCharacter():Health()

	health:AddHediff("alcohol", 0, {severity = 0, effect = 20 * uses, tended_start = os.time(), tended_time = 120})
end