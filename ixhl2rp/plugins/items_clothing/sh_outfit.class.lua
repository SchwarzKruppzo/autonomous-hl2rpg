...
if SERVER then
	...
	function Outfit:ModelChanged(model, oldModel)
		...
		if self.owner:GetCharacter() != self.character then -- character is not fully loaded yet
			return
		end
	...
	function Outfit:Update()
		...
		-- Определяем скрытые слоты и body-маски за один проход
		local hiddenSlots = {}
		local bodyMasks = {}
		...
		-- Группируем видимую одежду по attachment
		local visible = {
		...
		for layer, part in ipairs(self.layers) do
			...
			-- Мерджим все визуалы (модель берём первую непустую)
			...
			-- Для базового тела добавляем маски скрытия
			if layer == 1 then
			...
			-- Установка модели (только если изменилась)
			if targetModel then
			...
			-- bodyGroups (заполняем недостающие нулями)
	...
	function Outfit:TemporaryHideParts()
		...
		for displayID, attachment in pairs(self.owner.model_parts) do
			print("hide model!", self.owner:GetName(), displayID, attachment)
			if IsValid(attachment) then
				SafeRemoveEntity(attachment)
			end
	...
	function Outfit:RevealParts()
		timer.Simple(0, function()
			...
			print("=== hiddenModels Reveal! ===", self.owner:GetName())
			PrintTable(self._hiddenModels)
...
/*
	hook.Add("OnEntityCreated", "appearance.pending", function(entity)
		local entIndex = entity:EntIndex()
		local pending = pendingUpdates[entIndex]
		...
		timer.Simple(0, function()
			if IsValid(entity) then
				...
				print("=== pending.added ===")
				PrintTable(pending.added)
				print("=== pending.removed ===")
				PrintTable(pending.removed)
				entity.char_outfit:Update(pending.added, pending.removed)
			end
		end)

		pendingUpdates[entIndex] = nil
	end)*/

	hook.Add("UpdateAnimation", "appearance.pending", function(entity)
		local outfit = entity.char_outfit
