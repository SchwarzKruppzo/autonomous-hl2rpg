...
function ENT:Use(pPlayer)
	...
	self:EmitSound("needs/valve/fill_sink" .. math.random(1, 2) .. ".wav", 70)

	net.Start("ixAnimEvent")
		net.WriteEntity(pPlayer)
		net.WriteInt(pPlayer:LookupSequence("bg_down"), 32)
	net.SendPVS(pPlayer:GetPos())
	...
	elseif pPlayer:HasItem("empty_tin_can") then
		...
		timer.Simple(1, function()
			...
			ix.chat.Send(pPlayer, "me", "пьёт воду из под крана.")
		end)
	end
end

properties.Add("sinktrigger", {
	MenuLabel = "Вода (кран)",
	Order = 400,
	...
	Receive = function(self, length, client)
		...
		local sink = ents.Create("ix_sink_trigger")
		sink:SetPos(pos)
		sink:SetAngles(ang)
		sink:Spawn()
	end
