...
function imgui.xButton(x, y, w, h, borderWidth, borderClr, hoverClr, pressColor )
	...
	local bgColor = ( ( imgui.IsHovering(x, y, w, h) and hoverClr ) ) or imgui.skin.backgroundHover or imgui.skin.background
...
function imgui.xTextButton(text, font, x, y, w, h, borderWidth, color, hoverClr, fontClr, pressColor )
	local fgColor =
	...
	local clicked = imgui.xButton(x, y, w, h, borderWidth, color, hoverClr, pressColor, pressFillColor)

	font = imgui.xFont(font, math.floor(h * 0.618))
	
	local fontColor = imgui.IsHovering(x, y, w, h) and fontClr or color

	draw.SimpleText(text, font, x + w / 2, y + h / 2, fontColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
