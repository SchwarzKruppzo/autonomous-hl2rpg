...
if (SERVER) then
	...
	function ix.storage.Sync(client, inventory)
		...
		if isfunction(info.OnSync) then
			info.OnSync(client)
		end
	...
	net.Receive("ixStorageMoneyTake", function(length, client)
		...
		if !inventory or !inventory.storageInfo or storageID != inventory.id then
			return
		end
	...
	net.Receive("ixStorageOpen", function()
		...
		if IsValid(entity) and inventory then
			local localInventory = LocalPlayer():GetInventory("main")
			...
			if entity:IsPlayer() then
				panel:SetPlayerInventory(entity)
			end
